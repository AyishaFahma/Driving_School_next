...
export default function Create({ showmodal, togglemodal, admissionData, isEditing = false, onAdmissionAdded }: AdmissionModalProps) {
  ...
  useEffect(() => {
    ...
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (branchDropdownRef.current && !branchDropdownRef.current.contains(event.target as Node)) {
        setisbranchDropdown(false); // Close branch dropdown
      }
    };
    //cleanup 
  ...
  //filtered branch data based on search
  useEffect(() => {
    //if user search a branch
    if (searchBranch) {
      const filtered = branches.filter((single_branch) => single_branch.text.toLowerCase().includes(searchBranch.toLowerCase()))
  ...
  //console.log("filtered branch",filteredBranches);

  // when a branch is selected it displays in the box
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const finalFormData = {
      ...
      profile: {
        ...
        branchId: admissionFormData.profile.branchId || selectedBranch
      },
      service: {
    ...
    // Document 1
    if (finalFormData.profile.documents[0]) {
      formDataToSend.append("doctype1", finalFormData.profile.documents[0].type || "");
      if (finalFormData.profile.documents[0].file) {
        formDataToSend.append("docfile1", finalFormData.profile.documents[0].file);
      }
    }

    // Document 2
    if (finalFormData.profile.documents[1]) {
      formDataToSend.append("doctype2", finalFormData.profile.documents[1].type || "");
      if (finalFormData.profile.documents[1].file) {
        formDataToSend.append("docfile2", finalFormData.profile.documents[1].file);
      }
    }
                    ...
                    <div className="mb-4 mt-4">
                      ...
                      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 mt-2">
                        ...
                        <div className="relative w-full" ref={branchDropdownRef} >
                          ...
                          {/* Dropdown Button */}
                          <div onClick={() => setisbranchDropdown(!isbranchDropdown)} className="text-sm pl-2 mt-1 flex w-full items-center justify-between rounded-md border border-slate-300 bg-white py-2 px-3 shadow-sm cursor-pointer focus:border-indigo-500 focus:outline-none focus:ring-indigo-500 sm:text-sm dark:border-navy-600 dark:bg-navy-700 dark:text-navy-100">
                            {/* display the selected branch here */}
                            {/* admissionFormData.profile.branchId || "Select a Branch" */}
