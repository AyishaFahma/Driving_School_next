...
const page = () => {
  ...
  const togglemodal = (
    ...
    fetchAccountData(); // after editing automatic update on this page
  };
  ...
  const fetchBranchData = async () => {
    try {
      const response = await fetch("/api/admin/settings/get_branches_data", {
        method: "POST",
        headers: {
          authorizations: state?.accessToken ?? "",
      ...
      const data = await response.json();
      console.log("branch data", data.message.data)
  ...
  useEffect(() => {
    if (searchBranch) {
      const filtered = BranchData.filter((item) => item.text.toLowerCase().includes(searchBranch.toLowerCase()))

      setFilteredBranch(filtered)
    }
    else {
      setFilteredBranch(BranchData)
    }
  }, [searchBranch, BranchData])


  // select branch in filter
  ...
  const applyFilters = () => {
    ...
    if (dailystatusselected) {
      newFilteredData = newFilteredData.filter((item) => item.daily_status === dailystatusselected);
    }
    if (selectedStatus) {
    ...
    if (selectedDate) {
      newFilteredData = newFilteredData.filter((item) => {
        // Extract only the date part from added_date (before the space) . alsom can use includes()
        const itemDateOnly = item.added_date.split(' ')[0];
    ...
    // Date filter is now handled by the API call via filteredDate
  ...
  const handleReset = async () => {
    ...
    setSelectedStatus("");
    setSelectedDate("");
    setFilteredDate(""); // Update filteredDate from today to ""  setSelectedDate(today); setFilteredDate(today);
    setSelectedBranch("");
                              ...
                              <div className="flex space-x-2">
                                ...
                                <button
                                  className={`btn size-8 p-0 ${item.status === "active"
                                    ? "text-success"
                                    : "text-primary"
                                    } hover:bg-${item.status === "active"
                                      ? "info"
                                      : "primary"
                                    }/20 focus:bg-${item.status === "active"
                                      ? "warning"
                                      : "primary"
                                    }/20 active:bg-${item.status === "active"
                                      ? "error"
                                      : "primary"
                                    }/25`}
