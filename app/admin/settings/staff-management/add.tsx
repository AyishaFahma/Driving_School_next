...
const Add: React.FC<CreateProps> = ({ showmodal, togglemodal, formData, isEditing }) => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const formDataToSend = { ...localFormData, branch_id: branch_id, password: password};
    ...
    try {
      ...
      if (data.message.success === true) {
        toast.success( data.message.msg ||'Staff Added successfully');
  ...
  const fetchSearchBranch = async () => {
    try {
      ...
      const data = await response.json();
      console.log("all branch name data", data);
